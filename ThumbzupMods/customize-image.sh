...
Main() {
	case $RELEASE in
		...
		focal)
			...
			;;
		jammy)
			sudo curl -fsSL https://apt-kiosk.thumbzup.com/repositories/pubkey.gpg | sudo gpg --dearmor -o /usr/share/keyrings/ThumbzupAPTVendingServer.gpg
			sudo echo "deb [arch=arm64 signed-by=/usr/share/keyrings/ThumbzupAPTVendingServer.gpg] https://apt-kiosk.thumbzup.com/repositories jammy main
" | sudo tee /etc/apt/sources.list.d/apt-kiosk.list
			sudo apt update
			sudo apt install -y ./tmp/overlay/smart-tupay-vending_1.2.2_arm64.deb
			setSmartTuPayCronJob
			;;
	esac
} # Main
...
setSmartTuPayCronJob() {
	...
	line="0 02 * * * root apt update && apt upgrade -y smart-tupay-vending"
	cron_file="/etc/crontab"

	# Check if the line already exists in the cron file
	grep -qF "$line" "$cron_file"

	if [ $? -ne 0 ]; then
	    # Append the line to the cron file
	    echo "$line" | sudo tee -a "$cron_file" >/dev/null
	    echo "Line added successfully to the system-wide cron job."
	else
	    echo "The line already exists in the system-wide cron job. No changes made."
	fi
